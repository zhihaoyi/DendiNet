...
class biLSTM(nn.Module):
    def __init__(self, configs):
        ...
        self.bi_lstm = nn.LSTM(self.num_features, self.hidden_unit, batch_first=True, num_layers=1, bidirectional=True)
    ...
    def forward(self, input):
        ...
        output, _ = self.bi_lstm(norm_input) # [B, L, 2*d]
        output = output.reshape(-1, 2*self.sequence_length*self.hidden_unit) # [B, 2*L*d]
